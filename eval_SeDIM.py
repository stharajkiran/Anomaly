...
def test(model,
    ...
    # if model is empty, build it. Else use the previous model
    if model.IsFilled == False:
        if train_data is not None:
            ...
            model.build_gaussian(img_paths, kwargs['first_k'])
            # save model
            model_path = os.path.join(model.model_dir, model.model_name)# Save the model
    ...
    # If descriptors are already saved
    ...
    for (idx, img_path, gt, label, img_name) in tqdm(dataloader):
        for (_id, i, g, l, n) in zip(idx, img_path, gt, label, img_name):
            _id = int(_id)
            mask = get_ground_truth_mask(g, l, kwargs['load_size'])
            if saved_test_info:
                # index = x.index(i)
    ...
    if not saved_test_info and False:
    ...
    scores = F.interpolate(scores.unsqueeze(1), size=kwargs['load_size'], mode='bilinear',
    ...
    scores_dic_folders = "result_SeDIM/scores"
    scores_pkl_dir = f"{scores_dic_folders}/facet_{model.facet}_layer_{model.layer}/{class_name}"
    scores_dict_filepath = os.path.join(scores_pkl_dir, "scores.pkl")

    if not os.path.exists(scores_dict_filepath):
        if not os.path.exists(scores_pkl_dir):
            os.makedirs(scores_pkl_dir, exist_ok=True)
        with open(scores_dict_filepath, 'wb') as f:
            pickle.dump(scores_info, f)
...
def main(args):
    ...
    model_name = f"model_{kwargs['class_name']}.pkl"
    ...
    if os.path.exists(model_path):
        ...
        # make a new model
        model = GaussianModel(extractor, layer, facet, kwargs['load_size'])
    ...
    metrics, scores_info = test(model, test_dataloader, is_vis=True, img_dir=img_dir, train_data=train_dataloader, **kwargs)
...
def get_args():
    ...
    parser.add_argument('--class-name', type=str, default='capsules')

    parser.add_argument('--layer', type=int, default=7)
    parser.add_argument('--k-shot', type=int, default=4)
    parser.add_argument("--experiment_indx", type=int, default=0)
    parser.add_argument('--first_k', type=int, default=20)


    parser.add_argument("--root-dir", type=str, default="./result_224")
