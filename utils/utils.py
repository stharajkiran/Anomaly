...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        # Get SAM mask from patchcore mask
        ...
        # Segmentation is the anamoly mask
        # Pass it to sam along with original image
        # Returns the SAM mask
        # Convert the plot to two rows

        savename = image_path.split("/")
        savename = savename[-1].split("\\")

        savename_str = "_".join(savename[-save_depth:])
        os.makedirs(os.path.join(savefolder,savename[-4]), exist_ok=True)
        savename_path = os.path.join(savefolder,savename[-4], savename_str)
