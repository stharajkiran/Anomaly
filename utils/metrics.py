...
def metric_cal(scores, gt_list, gt_mask_list, cal_pro=True):
    ...
    # AUROC
    img_roc_auc = roc_auc_score(gt_list,
                                img_scores)  # Compute Area Under the Receiver Operating Characteristic Curve (ROC AUC); Image level
    #
    ...
    # pixel AUROC
    per_pixel_rocauc = roc_auc_score(gt_mask.flatten(),
                                     scores.flatten())  # Compute Area Under the Receiver Operating Characteristic Curve (ROC AUC); Pixel level
    ...
    result_dict = {'i_AUROC': img_roc_auc * 100,
                   'AUPR': aupr * 100,
                   'i_F1_max': img_f1 * 100,

                   'p_AUROC': per_pixel_rocauc * 100,
                   'PRO': pro_auc_score * 100,  # cal_pro_metric
                   'p_F1_max': pxl_f1 * 100,

                   'r_f1': max_f1_region * 100,  # calculate_max_f1_region
                   'p_threshold': pxl_threshold,
...
def cal_pro_metric(labeled_imgs, score_imgs, fpr_thresh=0.3, max_steps=200):
    ...
    labeled_imgs = np.squeeze(labeled_imgs)
