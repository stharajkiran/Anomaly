...
class ViTExtractor:
    ...
    def preprocess(self, image_path: Union[str, Path],
        ...
        l = min(pil_image.size)
        pil_image = transforms.CenterCrop((l, l))(pil_image)

        resize = (load_size, load_size)
        if load_size is not None and resize is not None:
            pil_image = transforms.Resize(resize, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
            pil_image = transforms.CenterCrop(load_size)(pil_image)
